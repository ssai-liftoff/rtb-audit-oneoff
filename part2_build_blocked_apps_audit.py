...
def build_audit():
    ...
    for i, rtb_row in rtb_df.iterrows():
        ...
        # Derive which high-spend apps are blocked
        blocked_ids = get_blocked_app_ids(filter_type, result["app_ids"], all_high_spend_app_ids)

        if not blocked_ids:
            print(f"    → No blocked high-spend apps (filter_type: {filter_type})")
            continue

        print(f"    → {len(blocked_ids)} blocked high-spend apps (filter_type: {filter_type})")
        ...
        for app_id in blocked_ids:
            ...
            # Uplift = (RTB connection daily spend / app daily revenue) * app daily revenue
            # Simplified: proportional opportunity score
            uplift = rtb_daily_spend * app_daily_revenue
    ...
    output_df = output_df.sort_values("uplift_score", ascending=False).reset_index(drop=True)
    ...
    print(f"{'='*50}")
    print(f"\nTop 5 opportunities:")
    print(output_df[["rtb_connection_name", "blocked_app_name", "rtb_daily_spend", "app_daily_revenue", "uplift_score"]].head())

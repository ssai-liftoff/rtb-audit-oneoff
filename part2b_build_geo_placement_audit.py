...
def build_audit():
    ...
    for i, (_, row) in enumerate(rtb_df.iterrows()):
        connection_id = str(row["rtb_connection_id"])
        account_id = str(row["rtb_account_id"])

        print(f"  [{i+1}/{total_connections}] {row['rtb_connection_name']} ({connection_id})")
        ...
        # Union countries across connections
        ...
        # Union placement types across connections
